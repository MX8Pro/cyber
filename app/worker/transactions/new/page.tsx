...
export default async function WorkerNewTransactionPage() {
  const session = await getSessionUser();
  if (!session || session.role !== "worker" || !session.workerId) {
    return <WorkerOfflineRoute view="transaction" />;
  }
  ...
  return (
    <WorkerShell workerName={snapshot.worker.displayName} workerId={session.workerId!}>
      <WorkerTransactionScreen workerId={session.workerId!} initialSnapshot={snapshot} />
    </WorkerShell>
  );
}
